...
%% This function is to use n-fold to calculate the R value of CPM analysis (no normalization on behavior)
% Input:
...
%%%%% mengxia.gao@gmail.com, 20200818
%%%%% updated on 20201022
...
%     test_cov = all_cov(testinds,:);
    ...
    elseif regress_method == 2
            fit_pos = regress(train_behav, [ones(size(traininds,2),1), train_sumpos]);
            fit_neg = regress(train_behav, [ones(size(traininds,2),1), train_sumneg]);
            fit_both = regress(train_behav, [ones(size(traininds,2),1), train_sum_all]);
    ...
    for kk = 1: size(testinds,2)
        test_sumpos = sum(sum(test_mats(:,:,kk).*pos_mask, 'omitnan'));
        test_sumneg = sum(sum(test_mats(:,:,kk).*neg_mask, 'omitnan'));
        ...
        elseif regress_method == 2
            behav_pred_pos(kk) = fit_pos(1) + fit_pos(2)*test_sumpos;
            behav_pred_neg(kk) = fit_neg(1) + fit_neg(2)*test_sumneg;
            behav_pred_both(kk) = fit_both(1)+ fit_both(2)*test_sum_all(1);
    ...
    [R_pos, ~] = corr(behav_pred_pos,test_behav,'type', corr_type);
    [R_neg, ~] = corr(behav_pred_neg,test_behav,'type', corr_type);
    [R_both, ~] = corr(behav_pred_both,test_behav,'type', corr_type);
